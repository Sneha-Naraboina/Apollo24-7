...
//sixth scenario negative 

public void enterSearchInputsFromExcel2(int sheet, int row) throws IOException {
    String[] searchTerms = ExcelReader.getRowData(sheet, row);
    for (String term : searchTerms) {
        if (!term.trim().isEmpty()) {
            enterSearchInput(term);
    ...
    public boolean isNoResultsMessageDisplayed() {
        ...
        try {
            WebElement noResultsElement = wait.until(ExpectedConditions.visibilityOfElementLocated(
                By.xpath("/html/body/div[2]/div[2]/div[1]/div[2]/div[1]/div/div/div/div/div")
            ));
            boolean isVisible = noResultsElement.isDisplayed();
            System.out.println("No results message visibility: " + isVisible);
            return isVisible;
        } catch (Exception e) {
            System.out.println("No results message not found or not visible: " + e.getMessage());
            return false;
        }
    }
    }
